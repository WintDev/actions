...
jobs:
  build:
    ...
    uses: WintDev/actions/.github/workflows/build.test.upload.app.yml@v4
    with:
...
jobs:
  ...
  deploy:
    name: Deploy (MyApp)
    needs: build
    uses: WintDev/actions/.github/workflows/deploy-function-app.yml@v4
    with:
...
jobs:
  ...
  deploy:
    ...
    needs: build
    uses: WintDev/actions/.github/workflows/deploy-web-app.yml@v4
    with:
...
jobs:
  build_packages:
    ...
    uses: WintDev/actions/.github/workflows/build-pack-push-package.yml@v4
    with:
...
jobs:
  build:
    name: test-ubuntu
    uses: WintDev/actions/.github/workflows/test-app-ubuntu.yml@v4
    with:
      solution-name: Wint.MyService.sln
    secrets:
      nuget-read-pat: ${{ secrets.NUGET_READ_PAT }}
...
jobs:
  build:
    name: test-windows
    uses: WintDev/actions/.github/workflows/test-app-windows.yml@v4
    with:
      solution-name: Wint.MyService.sln
    secrets:
      nuget-read-pat: ${{ secrets.NUGET_READ_PAT }}
